...
describe('Google Workspace group checks', () => {
  ...
  it('denies access and warns through the shared logger when the group cannot be resolved', async () => {
    process.env.GOOGLE_WORKSPACE_GROUP = 'missing@wengo.com';
    mockLookup.mockResolvedValue({ data: {} });

    await expect(checkGroupMembership('access-token', 'user@wengo.com')).resolves.toBe(false);

    expect(mockCheckTransitiveMembership).not.toHaveBeenCalled();
    expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining('missing@wengo.com'));
  });

  it('logs through the shared logger and rethrows when Cloud Identity fails', async () => {
    process.env.GOOGLE_WORKSPACE_GROUP = 'librechatadmins@wengo.com';
    const apiError = new Error('Cloud Identity API has not been used in project');
    mockCheckTransitiveMembership.mockRejectedValue(apiError);

    await expect(checkGroupMembership('access-token', 'user@wengo.com')).rejects.toBe(apiError);
